...
pub fn execute_command(command: Commands, workspace_path: Option<&Path>) -> Result<()> {
    match command {
        ...
        Commands::Diff {
            ...
            to,
            json,
        } => thin_wrapper_diff_command(workspace_path, &source, &from, &to, json),
        Commands::Export {
            input,
            file,
            to,
...
fn init_command(workspace_path: Option<&Path>, from_global: bool) -> Result<()> {
    ...
    rt.block_on(async {
        ...
        let workspace =
            SnapbaseWorkspace::create_new(root.to_path_buf(), storage_backend, config.clone())
                .await?;
...
fn export_command(
    ...
    if dry_run {
        ...
        println!("  Output: {output_file} ({export_format:?} format)");
        return Ok(());
    }

    // Ask for confirmation if not forced and file exists
    if output_path.exists() && !force {
        println!("⚠️  Output file '{output_file}' already exists. Continue? (y/N)");
...
/// Create a snapshot
fn snapshot_command(workspace_path: Option<&Path>, input: &str, name: Option<&str>) -> Result<()> {
    let workspace = SnapbaseWorkspace::find_or_create(workspace_path)?;
    ...
    let metadata = create_hive_snapshot(&workspace, &input_path, input, &snapshot_name)?;
    ...
    let hive_display_path = std::path::Path::new("sources")
        ...
        .join(format!(
            "snapshot_timestamp={}/",
            metadata.created.format("%Y%m%dT%H%M%S%.6fZ")
        ));
...
fn database_snapshot_command(
    ...
    for table in tables {
        ...
        let snapshot_name = if let Some(prefix) = name_prefix {
            ...
            namer.generate_name(
                &format!("{database_name}_{table_name}"),
                &existing_snapshots,
            )?
        };
...
fn create_table_snapshot(
    ...
    let metadata =
        create_hive_snapshot(workspace, &sql_file_path, &source_identifier, snapshot_name)?;
...
fn show_command(
    ...
    let metadata = if let Some(preloaded) = resolved.get_metadata() {
        ...
        let rt = tokio::runtime::Runtime::new()?;
        let metadata_data =
            rt.block_on(async { workspace.storage().read_file(json_path).await })?;
    ...
    if let Some(obj) = metadata_json.as_object_mut() {
        ...
        obj.insert(
            ...
            serde_json::Value::String(schema_hash),
        );
    }
...
fn cleanup_command(
    ...
    println!("   • Total archives: {full_archives_count}");
    println!(
        "   • Snapshots for data cleanup: {}",
        candidates_for_cleanup.len()
    );
    println!("   • Keep full data for: {keep_full}");
    ...
    if !force {
        ...
        if !candidates_for_cleanup.is_empty() {
            ...
            println!(
                "   • Rollback will NOT work for snapshots: {} to {}",
                oldest_cleanup.name, newest_cleanup.name
            );
        }
        println!("   • Rollback WILL work for the {keep_full} most recent snapshots");
        println!(
            "   • Full data will be removed from {} snapshots (deltas preserved)",
            candidates_for_cleanup.len()
        );
...
/// Query historical snapshots using SQL (workspace-wide only)
fn query_command(
    workspace_path: Option<&Path>,
    sql: Option<&str>,
    format: &str,
...
fn print_csv_result(result: &QueryResult) -> Result<()> {
    ...
    // Print rows
    for row in &result.rows {
        let csv_row: Vec<String> = row.iter().map(format_query_value).collect();
...
fn create_hive_snapshot(
    ...
    println!(
        "📊 Found {} rows, {} columns",
        data_info.row_count,
        data_info.column_count()
    );

    if data_info.row_count > 100_000 {
        println!(
            "🚀 Using optimized streaming for large dataset ({} rows)...",
            data_info.row_count
        );
    }

    // Finish schema analysis phase
    progress_reporter.finish_schema(&format!(
        "Schema analyzed: {} columns",
        data_info.column_count()
    ));
    ...
    if sql::is_sql_file(input_path) {
        ...
        if !select_query.is_empty() {
            ...
            let copy_sql = format!("COPY ({select_query}) TO '{parquet_path}' (FORMAT parquet)");
    ...
    let upload_progress =
        progress_reporter.create_upload_progress(metadata_size, "Uploading metadata...");

    rt.block_on(async {
        workspace
            .storage()
            .write_file_with_progress(&metadata_path, metadata_bytes, upload_progress)
            .await
    })?;
...
fn configure_storage(
    ...
    let config = match backend {
        crate::cli::StorageBackend::Local => {
            let path = local_path
                .clone()
                .unwrap_or_else(|| ".snapbase".to_string());
            StorageConfig::Local {
        ...
        crate::cli::StorageBackend::S3 => {
            ...
            let use_express = s3_express
                || std::env::var("SNAPBASE_S3_USE_EXPRESS")
                    .map(|v| v.to_lowercase() == "true")
                    .unwrap_or(false);
            let availability_zone = if use_express {
                let az = s3_availability_zone
                    .clone()
    ...
    if global {
        ...
        if workspace_config_path.exists() {
            ...
            let workspace =
                snapbase_core::workspace::SnapbaseWorkspace::find_or_create(Some(root))?;
...
fn stats_command(workspace_path: Option<&Path>, json: bool) -> Result<()> {
    ...
    let stats = rt.block_on(async {
        ...
        Ok::<snapbase_core::workspace::WorkspaceStats, SnapbaseError>(
            snapbase_core::workspace::WorkspaceStats {
                snapshot_count,
...
fn thin_wrapper_status_command(
    ...
    use snapbase_core::change_detection::{ComparisonOptions, DataSource};
    ...
    if !json {
        println!(
            "📊 Streaming comparison of '{}' against snapshot '{}'...",
            input, comparison_snapshot.name
        );
    }

    // Get the actual data path from the snapshot
    let baseline_data_path = comparison_snapshot
        .data_path
        .as_ref()
    ...
    if json {
        println!(
            "{}",
            JsonFormatter::format_comprehensive_status_results(&result)?
        );
    } else {
...
mod tests {
    ...
    fn test_init_command() {
        ...
        assert!(
            result.is_ok(),
            "Should handle already initialized workspace"
        );
