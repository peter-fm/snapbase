...
use snapbase_core::{workspace::SnapbaseWorkspace, ExportFormat, ExportOptions, UnifiedExporter};
use std::path::Path;
...
fn test_export_format_detection() {
    ...
    assert_eq!(
        ExportFormat::from_extension(Path::new("test.csv")).unwrap(),
        ExportFormat::Csv
    );
    assert_eq!(
        ExportFormat::from_extension(Path::new("test.parquet")).unwrap(),
        ExportFormat::Parquet
    );
    assert_eq!(
        ExportFormat::from_extension(Path::new("test.json")).unwrap(),
        ExportFormat::Json
    );
    assert_eq!(
        ExportFormat::from_extension(Path::new("test.xlsx")).unwrap(),
        ExportFormat::Excel
    );
...
fn test_export_options() {
    ...
    assert!(!custom_options.include_header);
