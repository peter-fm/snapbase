...
fn test_complete_cli_workflow() {
    ...
    let cli_path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../target/release/snapbase");
    ...
    assert!(
        snapshot1_output.status.success(),
        "Baseline snapshot creation failed: {}",
        String::from_utf8_lossy(&snapshot1_output.stderr)
    );
    ...
    assert!(
        status1_output.status.success(),
        "Status check failed: {}",
        String::from_utf8_lossy(&status1_output.stderr)
    );
    ...
    assert!(
        snapshot2_output.status.success(),
        "Snap1 snapshot creation failed: {}",
        String::from_utf8_lossy(&snapshot2_output.stderr)
    );
    ...
    assert!(
        status2_output.status.success(),
        "Second status check failed: {}",
        String::from_utf8_lossy(&status2_output.stderr)
    );
    ...
    assert!(
        snapshot3_output.status.success(),
        "Snap2 snapshot creation failed: {}",
        String::from_utf8_lossy(&snapshot3_output.stderr)
    );
    ...
    let expected_content =
        fs::read_to_string(workspace.copy_data_file("employees_snapshot2.csv", "expected.csv"))
    ...
    assert!(
        query_filtered_output.status.success(),
        "Filtered query failed: {}",
        String::from_utf8_lossy(&query_filtered_output.stderr)
    );
...
fn test_diff_operations(cli_path: &std::path::Path, workspace_path: &std::path::Path) {
    ...
    assert!(
        ...
        "Diff baseline->snap1 failed: {}",
        String::from_utf8_lossy(&diff1_output.stderr)
    );
    ...
    assert!(
        ...
        "Diff snap1->snap2 failed: {}",
        String::from_utf8_lossy(&diff2_output.stderr)
    );
    ...
    assert!(
        diff3_output.status.success(),
        "Diff baseline->snap2 failed: {}",
        String::from_utf8_lossy(&diff3_output.stderr)
    );
...
fn test_cli_workflow_error_handling() {
    ...
    assert!(
        !bad_snapshot_output.status.success(),
    ...
    assert!(
        !bad_diff_output.status.success(),
...
fn test_cli_workflow_edge_cases() {
    ...
    assert!(
        empty_snapshot_output.status.success(),
        "Should handle empty CSV files: {}",
        String::from_utf8_lossy(&empty_snapshot_output.stderr)
    );
    ...
    if !snapshot2_output.status.success() {
        ...
        assert!(
            !stderr.is_empty(),
            "Should provide error message for duplicate snapshot"
        );
    }
}
