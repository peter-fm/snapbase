...
mod tests {
    ...
    fn test_row_change_detection() {
        ...
        let baseline_changed_data: HashMap<u64, Vec<String>> = changed_rows.baseline_changed.iter()
            .filter_map(|&idx| baseline_data.get(idx as usize).map(|row| (idx, row.clone())))
            .collect();
            
        let current_changed_data: HashMap<u64, Vec<String>> = changed_rows.current_changed.iter()
