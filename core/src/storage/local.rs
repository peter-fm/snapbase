...
impl StorageBackend for LocalStorage {
    ...
    async fn write_file_with_progress(&self, path: &str, data: &[u8], progress_bar: Option<&ProgressBar>) -> Result<()> {
        ...
        for chunk in data.chunks(chunk_size) {
            ...
            if let Some(pb) = progress_bar {
                ...
                if total_size > 1024 * 1024 { // Only delay for files > 1MB
    ...
    async fn list_snapshots(&self, source: &str) -> Result<Vec<SnapshotMetadata>> {
        ...
        let snapshot_names = self.list_directories(&source_path.to_string_lossy()).await?;
        let mut snapshots = Vec::new();
        
        for snapshot_name in snapshot_names {
            if let Some(_name) = snapshot_name.strip_prefix("snapshot_name=") {
                let snapshot_dir = source_path.join(&snapshot_name);
                let timestamps = self.list_directories(&snapshot_dir.to_string_lossy()).await?;
                ...
                for timestamp in timestamps {
                    if let Some(_ts) = timestamp.strip_prefix("snapshot_timestamp=") {
                        ...
                        if let Ok(metadata_data) = self.read_file(&metadata_path.to_string_lossy()).await {
    ...
    async fn list_all_snapshots(&self) -> Result<Vec<SnapshotMetadata>> {
        ...
        self.traverse_sources_directory("sources", &mut sources_with_snapshots).await?;
        ...
        while let Some(path) = dirs_to_check.pop() {
            ...
            for dir in dirs {
                ...
                if has_snapshots {
                    ...
                    let source_path = dir_path_str.strip_prefix("sources/").unwrap_or(&dir_path_str);
                    snapshot_names.sort();
