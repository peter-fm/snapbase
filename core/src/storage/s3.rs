    ...
    pub async fn new(
        ...
        if use_express {
            if let Some(ref az) = availability_zone {
                let endpoint = format!("https://s3express-{az}.{region}.amazonaws.com");
                log::info!("🚀 Using S3 Express endpoint: {endpoint}");
                config_builder = config_builder.endpoint_url(endpoint);
            } else {
                return Err(anyhow::anyhow!(
                    "Availability zone is required when using S3 Express"
                ));
            }
        }
        ...
        match client.head_bucket().bucket(&actual_bucket).send().await {
            ...
            Err(e) => {
                ...
                if e.to_string().contains("dispatch failure") {
                    error_details.push(
                        "This is likely a network connectivity or credential issue:".to_string(),
                    );
                    ...
                    error_details
                        .push("   - AWS_SECRET_ACCESS_KEY environment variable".to_string());
...
impl StorageBackend for S3Storage {
                ...
                Ok(_) => {
                    if let Some(pb) = progress_bar {
                        pb.finish_with_message(format!(
                            "✅ S3 upload complete: s3://{bucket_name}/{key}"
                        ));
                    }
    ...
    async fn list_directories(&self, path: &str) -> Result<Vec<String>> {
        ...
        log::debug!(
            "📋 Listing S3 directories - bucket: {}, prefix: {}, path: {}",
            bucket_name,
            prefix,
            path
        );
        ...
        let response = self
            ...
            .map_err(|e| {
                log::error!(
                    "❌ S3 ListObjects failed for bucket '{bucket_name}', prefix '{prefix}': {e}"
                );
                anyhow::anyhow!(
                    "Failed to list S3 directories in bucket '{}' with prefix '{}': {}",
                    bucket_name,
                    prefix,
                    e
                )
