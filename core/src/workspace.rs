...
use crate::storage::{StorageBackend, LocalStorage, S3Storage};
use std::fs;
...
impl SnapbaseWorkspace {
    ...
    pub fn find_or_create(start_dir: Option<&Path>) -> Result<Self> {
        ...
        rt.block_on(async {
            ...
            if let Some(workspace) = Self::find_existing(start, storage_backend.clone(), config.clone())? {
                return Ok(workspace);
            }
    ...
    fn find_existing(start_dir: &Path, storage_backend: Arc<dyn StorageBackend>, config: StorageConfig) -> Result<Option<Self>> {
        match config {
            StorageConfig::Local { .. } => {
                ...
                loop {
                    ...
                    if snapbase_dir.exists() && snapbase_dir.is_dir() {
                        return Ok(Some(Self::from_root(current.to_path_buf(), storage_backend, config)?));
                    }
            ...
            StorageConfig::S3 { .. } => {
                ...
                if snapbase_dir.exists() && snapbase_dir.is_dir() {
                    Ok(Some(Self::from_root(start_dir.to_path_buf(), storage_backend, config)?))
                } else {
                    Ok(None)
                }
            }
    ...
    pub async fn create_new(root: PathBuf, storage_backend: Arc<dyn StorageBackend>, config: StorageConfig) -> Result<Self> {
    ...
    pub fn from_root(root: PathBuf, storage_backend: Arc<dyn StorageBackend>, config: StorageConfig) -> Result<Self> {
            ...
            match self.storage_backend.list_all_snapshots().await {
                Ok(snapshots) => {
                    ...
                    for snapshot in snapshots {
                        ...
                        let is_same_source = if let Some(snapshot_source_path) = &snapshot.source_path {
...
# AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY"#,
    ...
    fn generate_databases_section(&self, databases: &std::collections::HashMap<String, crate::config::DatabaseConfig>) -> String {
...
# tables = [\"*\"]
# exclude_tables = [\"temp_*\"]".to_string();
        }
        ...
        for (name, db_config) in databases {
            ...
            if !db_config.exclude_tables.is_empty() {
                section.push_str(&format!("\nexclude_tables = {:?}", db_config.exclude_tables));
            }
...
mod tests {
    ...
    async fn test_snapshot_paths() {
        ...
        let config = StorageConfig::default();
        let storage_backend = create_storage_backend(&config).await.unwrap();
        let workspace = SnapbaseWorkspace::from_root(temp_dir.path().to_path_buf(), storage_backend, config).unwrap();
