...
impl DatabaseConnection {
    ...
    fn execute_discovery_query(&self, query: &str) -> Result<Vec<TableInfo>> {
        ...
        let sql_file_path = temp_dir.join(format!("snapbase_discovery_{}.sql", 
            std::process::id()));
        ...
        connection.execute(&attach_statement, []).map_err(|e| {
            crate::error::SnapbaseError::invalid_input(format!(
                ...
                self.config.database.as_ref().unwrap_or(&"unknown".to_string()),
                e
            ))
        })?;
    ...
    pub fn create_attach_statement(&self, alias: &str) -> String {
        match self.config.db_type {
            DatabaseType::Mysql => {
                format!("ATTACH '{}' AS {} (TYPE mysql);", self.connection_string, alias)
            }
            DatabaseType::Postgresql => {
                format!("ATTACH '{}' AS {} (TYPE postgres);", self.connection_string, alias)
            }
            DatabaseType::Sqlite => {
                format!("ATTACH '{}' AS {} (TYPE sqlite);", self.connection_string, alias)
            }
        }
    }
}
