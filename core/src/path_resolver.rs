...
impl PathResolver {
    ...
    pub fn new(workspace_root: PathBuf) -> Result<Self, SnapbaseError> {
        let current_working_dir = std::env::current_dir()
            .map_err(|e| SnapbaseError::invalid_input(&format!("Cannot get current directory: {}", e)))?;
    ...
    pub fn make_relative_to_workspace<P: AsRef<Path>>(&self, absolute_path: P) -> Option<PathBuf> {
        ...
        path.strip_prefix(&self.workspace_root).ok().map(|p| p.to_path_buf())
    }
...
mod tests {
    ...
    fn test_relative_workspace_root() {
        ...
        assert_eq!(resolver.workspace_root(), current_dir.join("test_workspace"));
    }
    ...
    fn test_get_hive_path() {
        ...
        assert_eq!(hive_path, "sources/test.csv/snapshot_name=v1/snapshot_timestamp=20240101T120000Z");
    }
