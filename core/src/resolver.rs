...
impl SnapshotResolver {
    ...
    pub fn resolve_by_name_for_source(&self, name: &str, source_file: Option<&str>) -> Result<ResolvedSnapshot> {
            ...
            if let Some(source_file) = source_file {
                ...
                all_snapshots.into_iter().find(|s| {
                    ...
                    if let Some(snapshot_source) = &s.source_path {
                        ...
                        let filename_match = source_path.file_name() == Path::new(snapshot_source).file_name();
                        // Relative path match
            ...
            let source_relative_path = if let Some(source_path) = &snapshot_metadata.source_path {
                if let Ok(canonical_source) = Path::new(source_path).canonicalize() {
                    if let Ok(canonical_root) = self.workspace.root.canonicalize() {
                        canonical_source.strip_prefix(&canonical_root)
                            .map(|p| p.to_string_lossy().to_string())
                            .unwrap_or_else(|_| {
                                // Fallback to filename if path stripping fails
                                Path::new(source_path).file_name()
    ...
    pub fn resolve_by_date_for_source(&self, date_str: &str, source_file: Option<&str>) -> Result<ResolvedSnapshot> {
        let target_date = parse_date_string(date_str)?;
        
        // Get all snapshots with metadata
        let rt = tokio::runtime::Runtime::new()?;
        let all_snapshots = rt.block_on(async {
            self.workspace.storage().list_all_snapshots().await
        }).map_err(|e| SnapbaseError::data_processing(format!("Failed to list snapshots: {e}")))?;
        ...
        for snapshot_metadata in all_snapshots {
            ...
            if matches_source && snapshot_metadata.created <= target_date {
                match &best_snapshot {
                    None => {
                        best_snapshot = Some((snapshot_metadata.name.clone(), snapshot_metadata.created));
                    }
                    Some((_, best_date)) => {
...
impl ResolvedSnapshot {
    ...
    pub fn require_archive(&self) -> Result<&PathBuf> {
        self.archive_path
            .as_ref()
            .ok_or_else(|| SnapbaseError::archive(format!("Archive not found for snapshot '{}'", self.name)))
    }
