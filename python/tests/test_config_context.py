...
class TestConfigContextFailureCases:
    """Test cases that should demonstrate current bugs/failures"""
    
    @pytest.mark.xfail(reason="This test documents the current bug - should fail until fixed")
    def test_current_bug_workspace_ignores_own_config(self):
        """
        This test documents the current bug and should FAIL until we fix it.
        
        The bug: workspace operations may use current directory config instead of 
        workspace-specific config when the workspace is created with an explicit path.
        """
        with tempfile.TemporaryDirectory() as temp_dir:
            # Create different configs in parent vs child directories
            parent_dir = Path(temp_dir)
            child_dir = parent_dir / "child_workspace"
            child_dir.mkdir()
            
            # Parent config (should NOT be used)
            with open(parent_dir / "snapbase.toml", 'w') as f:
                f.write('[storage]\nbackend = "local"\npath = "WRONG_PARENT_PATH"\n')
            
            # Child config (should be used)
            with open(child_dir / "snapbase.toml", 'w') as f:
                f.write('[storage]\nbackend = "local"\npath = "CORRECT_CHILD_PATH"\n')
            ...
            try:
                ...
                workspace = Workspace(str(child_dir))
                config_info = json.loads(workspace.get_config_info())
                
                # This assertion should PASS (config resolution should work)
                assert config_info["config_source"] == "workspace"
                assert "child_workspace" in config_info["config_path"]
                
                # BUT: The actual bug might be in operations, not config resolution
                # So we test if operations actually use the right config context
                
                # If the bug exists, operations might use parent config instead of child config
                # This would manifest in subtle ways during actual snapshot operations
